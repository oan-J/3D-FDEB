(function () {
	d3.ForceEdgeBundling = function () {
		...
		function project_point_on_line(p, Q) {
			...
			var r = ((Q.source.y - p.y) * (Q.source.y - Q.target.y) - 
                (Q.source.x - p.x) * (Q.target.x - Q.source.x) + 
                (Q.source.z - p.z) * (Q.source.z - Q.target.z)) / (L * L);
		...
		function filter_self_loops(edgelist) {	
			...
			for (var e = 0; e < edgelist.length; e++) {
				console.log("edgelist[e]",edgelist[e])
				console.log("data_nodes[edgelist[e].target]",data_nodes[edgelist[e].target])
				console.log("data_nodes[edgelist[e].target].x",data_nodes[edgelist[e].target].x)
